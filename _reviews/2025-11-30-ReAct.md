...
# Takeways
- In a generic setup of an agent alternating between taking actions and receiving observations, ReAct augments the agent's action space by allowing the agent to think is the language space before outputting an action. This thinking space allows the agent to reason over the current context.
- The authors claim that the ReAct frameworks is intuitive and easy to deisgn, general and flexible, preformant and robust, and human aligned and controllable. 
